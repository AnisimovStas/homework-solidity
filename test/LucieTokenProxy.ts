...
describe("LucieToken", () => {
    ...
    it("initializa should be able to call once", async () => {
        const { tokenV2 } = await deployLucieTokenContractV2()
        await expect(tokenV2.initializeV2()).to.be.revertedWithCustomError(tokenV2, "InvalidInitialization()");
    });

    it("should fix safeMint to onlyOwner at V2 contract upgrade", async () => {
        const { tokenV1, owner, user } = await deployLucieTokenContractV1();

        await tokenV1.connect(user).safeMint(user.address, 1, "kek", { value: ethers.parseUnits("0.0001", "ether") })
        const LucieTokenV2 = await ethers.getContractFactory("LucieTokenV2");
        const tokenV2 = await upgrades.upgradeProxy(await tokenV1.getAddress(), LucieTokenV2);

        await tokenV2.connect(owner).initializeV2();
        await expect(tokenV1.connect(user).safeMint(user, 3, "kek", { value: ethers.parseUnits("0.0001", "ether") })).to.be.rejected;
    })


    it("should burn mined token and refound 90%", async () => {
        const { tokenV2, user } = await deployLucieTokenContractV2()
        await tokenV2.connect(user).safeMintWithAutoIncrement({ value: ethers.parseUnits("0.0001", "ether") });
        const balanceBeforeBurn = await getAccountBalance(user.address);
        await tokenV2.connect(user).burn(1);
        const balanceAfterBurn = await getAccountBalance(user.address);
        expect(balanceAfterBurn).to.be.greaterThan(balanceBeforeBurn)
